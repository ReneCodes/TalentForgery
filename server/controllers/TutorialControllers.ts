...
export async function createTutorial(req: Request, res: Response) {
  ...
  const { title, video_url, description, question_ids, questions_shown, access_date, due_date }: createdTutorial = req.body;

  if (!title || !video_url || !description || !question_ids || !questions_shown || !access_date || !due_date) {
